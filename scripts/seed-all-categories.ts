...
function parseQuestionsFromText(text: string): ParsedQuestion[] {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.match(/^​?[A-Da-d]\./)) {
            ...
            const normalizedLine = line.replace(/^​?([a-d])\./, (match, letter) => letter.toUpperCase() + '.');
            currentOptions.push(normalizedLine.replace(/^​/, '').replace(/​/g, '').trim());
            continue;
        }

        // Detect correct answer (both uppercase and lowercase)
        if (line.match(/Answer:\s*[A-Da-d]/)) {
            const match = line.match(/Answer:\s*([A-Da-d])/);
            if (match) {
                correctAnswer = match[1].toUpperCase(); // Normalize to uppercase
            }
            continue;
        }

        // Continue reading question text
        if (isReadingQuestion && line && !line.match(/^✅/) && !line.match(/^​ ​​/) && !line.match(/^\f/)) {
...
async function main() {
    try {
        ...
        console.log(`   • Each test picks 2 questions from each category`);
