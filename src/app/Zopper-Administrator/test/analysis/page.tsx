...
export default function QuestionAnalysisPage() {
  ...
  // For easiest, we take the ones with highest correctPercentage
  const easiestQuestions = [...analysis].sort((a, b) => b.correctPercentage - a.correctPercentage).slice(0, 5);
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-lg shadow p-5">
          <div className="text-3xl font-bold text-red-600">
            {hardestQuestions[0]?.questionNumber ? `Q${hardestQuestions[0].questionNumber}` : '-'}
          </div>
          <div className="text-sm font-semibold text-gray-700">Hardest Question</div>
        </div>
        <div className="bg-white rounded-lg shadow p-5">
          <div className="text-3xl font-bold text-purple-600">
            {easiestQuestions[0]?.questionNumber ? `Q${easiestQuestions[0].questionNumber}` : '-'}
          </div>
                  ...
                  <Fragment key={q.id}>
                    <tr className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-center font-bold text-orange-600">
                        {q.mostSelectedWrongOption !== '-' ? `Option ${q.mostSelectedWrongOption}` : '-'}
                      </td>
                      <td className="px-4 py-3 text-center">
                        <button
                        ...
                        <td colSpan={6} className="px-4 py-4 bg-gray-50">
                          <div className="space-y-2">
                            ...
                            {q.options && q.options.length > 0 ? q.options.map(opt => (
                              ...
                              <p className="text-gray-500 italic text-sm">No option details available.</p>
                            )}
                          </div>
                        </td>
                      </tr>
